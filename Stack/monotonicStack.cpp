...
// Example: Find the next smaller element to the left for each element in the array
...
vector<int> nextSmallerToLeft(vector<int>& nums) {
  ...
  for(int i = 0; i < n; i++) { // Traverse from left to right
    ...
    // If stack is not empty then top is the next smaller, else -1 (already initialised)
    if(!st.empty()) {
      ans[i] = st.top();
    }
...
// INTUITION: Stack stores potential "next smaller" elements. We scan from left to right. Greater elements are useless for future ones ➝ we pop them. Stack is now a monotonically increasing.


// For next smaller element to right just traverse from right to left with same logic
...
// INTUITION: Stack stores potential "next greater" elements. We scan from right to left. Smaller elements are useless for future ones ➝ we pop them. Stack is now a monotonically decreasing.

// For next greater element to the left traverse from left to right with same logic
